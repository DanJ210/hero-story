...
The first likeness phase now owns consent, private portrait metadata, replacement/disablement, and deletion. Phase 2 records an opaque portrait ID and consent timestamp on opted-in manual artwork jobs; the worker resolves the private blob, validates provenance, enforces bounded provider-reference age, normalizes source encoding, and sends the image directly to the provider's manual image-edit endpoint. Automatic likeness generation remains disabled.
