...
[ApiController]
[Authorize]
[Route("api/sessions/{id:guid}/scenes")]
public class ScenesController : ControllerBase
{
    ...
    [HttpGet]
    public Task<IReadOnlyList<SceneListDto>> GetScenes(Guid id, CancellationToken cancellationToken) => _sceneService.GetScenesAsync(GetUserId(), id, cancellationToken);

    [HttpPost]
    [EnableRateLimiting("scenes")]
    public async Task<ActionResult<SceneDto>> CreateScene(Guid id, CreateSceneRequest request, CancellationToken cancellationToken)
    {
        var response = await _sceneService.CreateSceneAsync(GetUserId(), id, request, cancellationToken);
